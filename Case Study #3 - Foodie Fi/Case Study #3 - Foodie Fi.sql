CREATE SCHEMA 8_week3_foodie_fi;
...
SELECT * FROM plans;
-- How many customers has Foodie-Fi ever had?
SELECT count(distinct customer_id) as TotalCustomers from subscriptions;

-- What is the monthly distribution of trial plan start_date values for our dataset - use the start of the month as the group by value
 SELECT month(start_date) as Month ,count(plan_id) as TotalplansbyMonth
 FROM subscriptions 
 WHERE plan_id = 0
 GROUP BY 1
 ORDER BY 1;
 ...
 -- What plan start_date values occur after the year 2020 for our dataset? Show the breakdown by count of events for each plan_name?
 SELECT p.plan_id, p.plan_name, count(s.customer_id) 
 FROM plans p
...
count(*) FROM subscriptions as s
INNER JOIN plans as p ON s.plan_id = p.plan_id
WHERE plan_name = "churn")
select *, round(CustomerCount/count(*),1) AS ChurnPercentage from COUNT_cte
GROUP BY 1;
-- ---------------- another method ----------------
SELECT
...
-- How many customers have upgraded to an annual plan in 2020?
SELECT  p.plan_name, count(distinct s.customer_id) as CountofCustomer
FROM plans p 
